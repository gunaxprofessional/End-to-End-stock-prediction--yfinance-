...
with DAG(
    'direction_classifier_model',
    default_args=default_args,
    description='Direction classifier model training pipeline',
    schedule='0 6 * * 1-5',  # Run at 6 AM on weekdays (after data pipeline)
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['model', 'direction', 'classifier', 'training'],
) as dag:
