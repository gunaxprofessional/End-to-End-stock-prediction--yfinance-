...
with DAG(
    ...
    description='Direction classifier model monitoring - update actuals and generate drift reports',
    schedule='0 18 * * 1-5',  # Run at 6 PM on weekdays (after market close)
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['model', 'direction', 'monitoring', 'drift'],
) as dag:
    ...
    update_actuals = DockerOperator(
        ...
        mount_tmp_dir=False,
        command='python -m models.direction_classifier.update_actuals',
    )
    ...
    run_monitor = DockerOperator(
        ...
        mounts=[ARTIFACT_MOUNT],
        mount_tmp_dir=False,
        command='python -m models.direction_classifier.monitor',
    )
